...
use App\Mail\resendRegisterOtpMail;
...
class VoterController extends Controller
{
    ...
    public function registerVoter(voterRequest $request)
    {
        ...
        if ($voter) {
            return response()->json(['message' => 'Voter already exists'], 400);
        }
        ...
        $token = $voter->createToken($voter->fullname)->plainTextToken;
    ...
    public function resendRegisterOtp(ResendRegisterOTPRequest $request)
    {
        ...
        $api_url = 'https://portal.nigeriabulksms.com/api/';
        $data = ['username' => env('SMS_USERNAME'), 'password' => env('SMS_PASSWORD'), 'sender' => env('SMS_SENDER_ID'), 'message' => $message, 'mobiles' => $phone];
        $data = http_build_query($data);
        $request = $api_url . '?' . $data;
        $result = file_get_contents($request);
        $result = json_decode($result);
        ...
        //send otp email
        Mail::to($voterRecord->email)->send(new resendRegisterOtpMail($otp,$voterRecord->fullname));
    ...
    public function login(loginRequest $request)
    {
        ...
        $voter = tblvoter::where('voterID', $request->voterID)
            ...
            if(Auth::attempt(['voterID' => $request->voterID])){
                $voter_data = Auth::tblvoter();
                $token =  $voter_data->createToken($voter->fullname)->plainTextToken;
                //$success['name'] =  $user->name;

                return response()->json(['success' => true, 'message' => 'Login Is successful and OTp sent to Email and Phone', 'token' => $token], 201);
            }
            else{
                return response()->json(['success' => false, 'message' => 'Login failed'], 201);
            }
    ...
    public function resendOtp(ResendOTPRequest $request)
    {
        ...
        //send otp via SMS
        $otp = rand(10200, 90002);
        $message = 'Hello, ' . $voterRecord->fullname . ' Your OTP to complete your Login process is ' . $otp . '. Please Do not share with anybody';
        $api_url = 'https://portal.nigeriabulksms.com/api/';
