...
use App\Mail\candidateregisterMail;
...
class CandidateController extends Controller
{
    public function registerCandidate(CandidateRequest $request)
    {
        ...
        $candidate = tblcandidate::create([
            ...
            'candidateName' => 'C' . rand(10200, 90002),
            'count' => 0,
       ...
       //tblvoter::where('voterID', $request->voterID)->first();
        ...
        //send email
        Mail::to($voter->email)->send(new candidateregisterMail($voter->fullname, $candidate->candidateID,$candidate->party, $candidate->office));
    ...
    public function getPresidentCandidateDetails()
    {
        $candidates = DB::table('tblcandidates')->join('tblvoters', 'tblcandidates.voterID', '=', 'tblvoters.voterID')->join('tblpartys', 'tblcandidates.party', '=', 'tblpartys.name')->where('tblcandidates.office', 'President')->select('tblcandidates.*', 'tblvoters.*', 'tblpartys.*')->get();
        if ($candidates) {
    ...
    {
        $candidates = DB::table('tblcandidates')->join('tblvoters', 'tblcandidates.voterID', '=', 'tblvoters.voterID')->join('tblpartys', 'tblcandidates.party', '=', 'tblpartys.name')->where('tblcandidates.office', 'Governor')->select('tblcandidates.*', 'tblvoters.*', 'tblpartys.*')->get();
        if ($candidates) {
            return response()->json($candidates, 201);
        }
    }
    public function getCandidateDetails(Request $request)
    {
        ...
        $candidate_data = DB::table('tblcandidates')->join('tblvoters', 'tblcandidates.voterID', '=', 'tblvoters.voterID')->where('tblcandidates.voterID', $voterID)->select('tblcandidates.*', 'tblvoters.*')->first();
        if ($candidate_data) {
