...
class VoterController extends Controller
{
    public function registerVoter(Request $request)
    {
        $request->validate([
         ...
         //$imageName=rand(10200, 90002).$request->image->getClientOriginalExtension;
         if ($request->hasFile('image')) {
            $image = $request->file('image');
            $imageName = rand(10200, 90002). '.'. $image->getClientOriginalExtension();
        } else {
            $imageName = 'default.jpg'; // or any other default value
        }
