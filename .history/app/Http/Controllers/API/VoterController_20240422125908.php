    ...
    public function registerVoter(Request $request)
{
    $request->validate([
        ...
        'photo' => 'required|string|max:5000', // Validate that the 'photo' field is an image file and has a maximum size of 5MB
    ]);

    // Check if a file was uploaded for the 'photo' field
    if ($request->hasFile('photo') && $request->photo instanceof UploadedFile) {
        $image = $request->photo;
