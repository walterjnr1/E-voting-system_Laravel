...
public function login(Request $request)
    {
        ...
        $request->validate([
            'voterID' => 'required|string|max:5',
        ]);

        $user = tblvoter::where('voterID', $request->voterID)
            ->where('status', 1)
            ->first();
...
//fetch user details
//$userDetails = tblvoter::where('voterID', $request->voterID)->first();


    $token = $user->createToken('Auth_login')->plainTextToken;
    return response()->json(['success' => true,'message' => 'Login Is successful','token'=> $token,'data'=>$user], 201);
    }
