...
{
    $candidateID = $request->input('search');

    $candidates = tblcandidate::where('office', 'President')
        ->where(function ($query) use ($candidateID, $voterID) {
            $query->where('candidateID', 'like', "%{$candidateID}%")
                ->orWhere('voterID', 'like', "%{$voterID}%");
        })
        ->where('status', 1)
        ->get();
